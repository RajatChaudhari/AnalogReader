...
def calibrate_gauge(gauge_number, file_type):
    ...
    cv2.imshow('galibrated image',img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    for i in range(0, len(lines)):
        for x1, y1, x2, y2 in lines[i]:
            ...
            #print (diff1<diff1UpperBound*r,diff1>diff1LowerBound*r,diff2<diff2UpperBound*r,diff2>diff2LowerBound*r)
    ...
    #test_value = ((new_range/old_range) * (old_value - old_min)) + new_min
    #test_value=get_value_from_angle(old_value,old_min)+ new_min
    # print ("***************  test_value ***************")
    # print (new_value)
    #return new_value
    return new_value
def get_value_from_angle(angle,old_min):
    quad_factor = {1:1,2:0.66,3:0.66,4:0.8}
    q=int(angle/90)
    r=angle%90
    r_val=r*quad_factor[q+1]
    if q==0:
        return angle - old_min
    elif q==1:
        q_val=(90-old_min)*quad_factor[1]
        return q_val+r_val
    elif q==2:
        q_val=(90-old_min)*quad_factor[1]+90*quad_factor[2]
        return q_val+r_val
    else:
        q_val=(90-old_min)*quad_factor[1]+90*quad_factor[2]+90*quad_factor[3]
        return q_val+r_val


def main():
    gauge_number = int(input('Enter image number : '))
    file_type='jpg'
    ...
    min_angle, max_angle, min_value, max_value, units, x, y, r = calibrate_gauge(3, file_type)
